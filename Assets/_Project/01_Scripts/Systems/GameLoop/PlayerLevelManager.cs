...
public class PlayerLevelManager : MonoSingleton<PlayerLevelManager>
{
    ...
    [Tooltip("각 레벨업에 필요한 경험치 (해당 레벨로 올라가기 위해 필요한 총 경험치)")]
    ...
    {
        currentExp += amount;
        Debug.Log($"경험치 {amount} 추가 후 현재 총 경험치 {currentExp}");

        CheckLevelUp();
    }

    /// <summary> 현재 레벨에서 다음 레벨로 올라가기 위해 필요한 경험치 </summary>
    ...
    /// <summary> 현재 레벨에서 획득한 경험치 (현재 레벨 내에서의 경험치) </summary>
